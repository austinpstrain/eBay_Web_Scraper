...
def get_image(soup): #Written by Austin
    try:
        img = soup.find('img', attrs ={'id':'icImg'})
        image = img.get('src')
    except:
        return ""
    return image

def get_category(soup): #Written by Austin
    try:
        script = soup.find('td', attrs = {"class":'vi-VR-brumblnkLst vi-VR-brumb-hasNoPrdlnks' , "style":"vertical-align:top;"})
        categories = script.find_all('span', attrs = {"itemprop":"name"})
    except:
        return ""
    if not categories:
        return ""
...
def get_brand(soup): #Written by Austin
    try:
        script = soup.find('h2', attrs ={"itemprop":"brand"})
        brand = script.find('span', attrs={"itemprop":"name"}).text
    except:
        return ""
    if not brand:
        return ""
...
def get_upc(soup): #Written by Austin
    """
    Returns product upc
    """
    try:
        item_upc = soup.find('h2', attrs = {'itemprop':'gtin13'})
        item_upc = item_upc.text
    except:
        item_upc = ""
    if str(item_upc) == "Does not apply" or str(item_upc) == "Does Not Apply":
        item_upc = ""
    return item_upc

def get_ean(soup): #written by Austin
    try:
        script = soup.find_all('h2', attrs = {'itemprop':'gtin13'})
        text = []
        for i in script:
            text.append(i.text)
        for i in text:
            if str(i).isdigit():
                if str(i) == "Does not apply" or i == 'Does Not Apply':
                    i == ""
                return i
...
def get_mpn(soup): #Written by Austin
    try:
        mpn = soup.find('h2', attrs = {'itemprop':'mpn'})
        mpn = mpn.text
    except:
        return ""
    if mpn == 'Does not apply' or mpn == 'Does Not Apply':
            mpn = ''
    return mpn

def get_isbn(soup): #Written by Austin
    try:
        isbn = soup.find('h2', attrs = {'itemprop':'productID'})
        isbn = isbn.text
        if isbn == 'Does not apply' or isbn == 'Does Not Apply':
            isbn = ''
    except:
        return ""
    return isbn
    
def get_payments(soup): #Written by Austin
    try:
        script = soup.find('div', attrs={'id':'payDet1'})
        payments =[]
        img = script.find_all('img')
    except:
        return ""
    for i in img:
        payments.append(i.get('title'))
    return payments


def get_items_sold(soup): #written by Austin
    try:
        sold = soup.find('a', attrs={'class':'vi-txt-underline'})
        sold = sold.text
        sold = sold.replace("sold","").replace(" ","")
        if sold[0].isalpha() == True or sold == "30-dayreturns" or sold == "60-dayreturns":
            sold = ""
    except:
        sold = ""
    try: #Attempt 2
        if sold == "" or sold == "None":
            sold = soup.find('span', attrs={'class':'w2b-sgl'})
            sold = sold.text
            sold = sold.replace("sold","").replace(" ","")
            if sold[0].isalpha() == True or sold == "30-dayreturns" or sold == "60-dayreturns":
                sold = ""
...
def get_price(soup): #written by Austin
    try:
        price = soup.find('span', attrs={'id':'prcIsum'})
        price = price.text.replace("/ea","")
        price = price[3:]
    except:
        price = ""
    try:
        if price == "":
            price = soup.find('span', attrs={'class':'notranslate'})
            price = price.text.replace("/ea","")
            price = price[3:]
    except:
        price = ""
    if price != '':
        price = price.replace('US','').replace(' ','').replace('$','')          
    return price
...
def get_item_location(soup):
    ...
    item_location = str(item_location).replace("b","").replace("'","")
