...
def collect_search_links(keyword): #Written by Austin
    base = "https://www.ebay.com/sch/i.html?_from=R40&_trksid=p2380057.m570.l1312.R1.TR0.TRC0.A0.H0.TRS2&_nkw="
    base_page = '&_pgn='
    ...
    results = soupy.select_one('.srp-controls__count-heading .BOLD:nth-child(1)')
    text_results = int(results.text.replace(',',''))
    
    max_pages=text_results/50
    if pages > max_pages:
        pages = int(max_pages)
    if pages == 0:
        pages = 1
    
    #Go through every page and grab the product URLS
    for i in range(pages):
        i += 1 #i index for range starts at 0
        soup = get_soup(base+keyword+base_page+str(i),'html.parser')
        productDivs = soup.findAll('div', attrs={'class' : "s-item__wrapper clearfix"})
